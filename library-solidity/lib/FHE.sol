...
library FHE {
    ...
    /// @return False if `user` has not (delegate, contractAddress) context, or if it has expired.
    ...
    function isHandleDelegatedForUserDecryption(
        ...
        uint64 expirationDate = Impl.getUserDecryptionDelegationExpirationDate(delegator, delegate, contractAddress);
        if (expirationDate < block.timestamp) {
            return false;
        }
        return isUserDecryptable(handle, delegator, contractAddress);
    }
    ...
    /// @dev This is the unlimited version of {delegateUserDecryption}. The permission remains active until explicitly
    ///      revoked by the delegator using {revokeUserDecryptionDelegation}.
    /// @param delegate The account that will request a user decryption on behalf of delegator (`msg.sender`).
    /// @param contractAddress The address of the contract that is part of the user decryption context.
    function delegateUnlimitedUserDecryption(address delegate, address contractAddress) internal {
        delegateUserDecryption(delegate, contractAddress, type(uint64).max);
    }
    ...
    function delegateUserDecryptions(address delegate, address[] memory contractAddresses, uint64 expiryDate) internal {
        ...
        for (uint256 i = 0; i < length; ++i) {
            Impl.delegateForUserDecryption(delegate, contractAddresses[i], expiryDate);
        }
    }

    /// @notice Batch delegates permanent user decryption rights that the caller (`msg.sender`) holds in the context of
    ///         the given `contractAddresses[i]` to a new `delegate` account.
    /// @param delegate The account that will request a user decryption on behalf of delegator (`msg.sender`).
    /// @param contractAddresses The array of contract addresses that form the user decryption context tuples
    ///                          (`msg.sender`, `contractAddresses[i]`).
    function delegateUnlimitedUserDecryptions(address delegate, address[] memory contractAddresses) internal {
        delegateUserDecryptions(delegate, contractAddresses, type(uint64).max);
    }
    ...
    function revokeUserDecryptionDelegations(address delegate, address[] memory contractAddresses) internal {
        uint256 length = contractAddresses.length;
        for (uint256 i = 0; i < length; ++i) {
            Impl.revokeDelegationForUserDecryption(delegate, contractAddresses[i]);
        }
    }
