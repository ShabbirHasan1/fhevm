    ...
    Publ: EventPublisher + 'static,
{
    ...
    async fn run(self) {
        ...
        tasks.spawn(self.subscribe_to_refresh_keygen_reshare());
    ...
    async fn subscribe_to_refresh_keygen_reshare(self) {
        let refresh_keygen_filter = self.kms_generation_contract.RefreshKeygenReshare_filter();
        self.subscribe_to_events(
            "RefreshKeygenReshare",
            refresh_keygen_filter,
            self.config.key_management_polling,
        )
        .await;
    }
...
mod tests {
    ...
    #[tokio::test]
    #[traced_test]
    async fn test_refresh_keygen_reshare_subscription() {
        let (asserter, gw_listener) = test_setup().await;

        // Used to mock a new event
        let rpc_event_log = mock_rpc_event_log(RefreshKeygenReshare::default());
        asserter.push_success(&[rpc_event_log]);

        tokio::spawn(gw_listener.subscribe_to_refresh_keygen_reshare());
        loop {
            if logs_contain("RefreshKeygenReshare published!") {
                break;
            }
            tokio::time::sleep(Duration::from_millis(100)).await;
        }
    ...
    impl EventPublisher for MockPublisher {
        async fn publish(&self, event: GatewayEvent) -> Result<()> {
            match event.kind {
                ...
                GatewayEventKind::RefreshKeygenReshare(_) => {
                    info!("RefreshKeygenReshare published!")
                }
            }
            Ok(())
        }
