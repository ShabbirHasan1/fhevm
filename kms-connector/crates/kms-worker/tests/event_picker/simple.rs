...
use fhevm_gateway_bindings::{
    ...
    kms_generation::KMSGeneration::{
        CrsgenRequest, KeygenRequest, PrepKeygenRequest, RefreshKeygenReshare,
    },
};
...
#[tokio::test]
async fn test_pick_refresh_keygen_reshare() -> anyhow::Result<()> {
    let test_instance = TestInstanceBuilder::db_setup().await?;
    ...
    info!("Triggering Postgres notification with RefreshKeygenReshare insertion...");
    sqlx::query!(
        "INSERT INTO refresh_keygen_reshare(prep_keygen_id, key_id, epoch_id, params_type, otlp_context) \
    ...
    info!("Picking RefreshKeygenReshare...");
    let events = event_picker.pick_events().await?;

    info!("Checking RefreshKeygenReshare data...");
    assert_eq!(
        events,
        vec![GatewayEvent {
            otlp_context: PropagationContext::empty(),
            kind: GatewayEventKind::RefreshKeygenReshare(RefreshKeygenReshare {
