...
use fhevm_gateway_bindings::{
    ...
    kms_generation::KMSGeneration::{
        CrsgenRequest, KeygenRequest, PRSSInit, PrepKeygenRequest, RefreshKeygenReshare,
    },
};
use sqlx::{
    Pool, Postgres, Row,
...
impl GatewayEvent {
    ...
    pub async fn mark_as_pending(&self, db: &Pool<Postgres>) {
        match &self.kind {
            ...
            GatewayEventKind::PrssInit(_) | GatewayEventKind::RefreshKeygenReshare(_) => (),
        }
    }
    ...
    pub async fn delete_from_db(&self, db: &Pool<Postgres>) {
        match &self.kind {
            ...
            GatewayEventKind::RefreshKeygenReshare(e) => {
                delete_refresh_keygen_reshare_from_db(db, e.keyId).await
            }
        }
    }
}
...
pub fn from_refresh_keygen_reshare_row(row: &PgRow) -> anyhow::Result<GatewayEvent> {
    let kind = GatewayEventKind::RefreshKeygenReshare(RefreshKeygenReshare {
        prepKeygenId: U256::from_le_bytes(row.try_get::<[u8; 32], _>("prep_keygen_id")?),
        keyId: U256::from_le_bytes(row.try_get::<[u8; 32], _>("key_id")?),
        epochId: U256::from_le_bytes(row.try_get::<[u8; 32], _>("epoch_id")?),
        paramsType: row.try_get::<ParamsTypeDb, _>("params_type")? as u8,
...
pub async fn delete_refresh_keygen_reshare_from_db(db: &Pool<Postgres>, id: U256) {
    let query = sqlx::query!(
        "DELETE FROM refresh_keygen_reshare WHERE key_id = $1",
        id.as_le_slice()
    );
    execute_delete_event_query(db, query).await;
}
...
impl Display for GatewayEventKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            GatewayEventKind::RefreshKeygenReshare(e) => {
                write!(f, "RefreshKeygenReshare #{}", e.keyId)
            }
        }
    }
}
...
impl From<RefreshKeygenReshare> for GatewayEventKind {
    fn from(value: RefreshKeygenReshare) -> Self {
        Self::RefreshKeygenReshare(value)
    }
}
